...
-- a → b, b → a ⊢ a ↔ b
example
    (ab : a → b) (ba : b → a) : Iff a b :=
  Iff.intro ab ba

-- a iff b
--   proves a implies b
-- a ↔ b ⊢ a → b
example (iff_ab : Iff a b) : a → b :=
  iff_ab.mp

-- a iff b
--   proves b implies a
-- a ↔ b ⊢ b → a
example (iff_ab : Iff a b) : b → a :=
  iff_ab.mpr
...
-- ⊢ (id a) = a
theorem id_eq (a : α) : Eq (id a) a :=
  Eq.refl a

-- ⊢ a = (id a)
theorem eq_id (a : α) : Eq a (id a) :=
  Eq.refl a
...
theorem Nat.not_succ_le_zero
    (n : Nat) : LE.le (succ n) zero → False :=
  have eq_m_zero_implies:
      (m : Nat) → Eq m zero →
        LE.le (succ n) m → False :=
